...
router.post('/upload', bodyParser(), function* () {
  var formData = yield formParser(this.req);
  let filenameArr = formData.filename.split('.');
  let suffix = filenameArr[filenameArr.length - 1];
  formData.filename = formData.filename.split(suffix)[0] + '_' + new Date().getTime() + '.' + suffix;
  var writeStream = origFs.createWriteStream(FilePath(formData.filename));
  formData.pipe(writeStream);
  this.body = formData.filename;
});
